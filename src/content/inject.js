(function () {
  ...
  function getActiveFilename(model) {
    ...
    // 1. Try to find via fileModelMap
    for (const [name, m] of fileModelMap.entries()) {
      if (m === model) {
        return name;
      }
    }

    // 2. Try to find the currently selected element in the project files listbox
    try {
      const fileList = document.querySelector('ul[role="listbox"][aria-label="Project files"]');
      if (fileList) {
        const activeItem = fileList.querySelector('li[role="option"][aria-selected="true"]') || 
    ...
    // 3. Fallback to model URI path if it's not a simple numeric string
    if (model.uri) {
